...
$allowedTypes = ['club', 'course', 'collaboration', 'personal_message', 'contact'];
...
if (in_array($type, ['course', 'collaboration', 'personal_message', 'contact'], true) && empty($input['service_consent'])) respond(422, ['message' => ['en' => 'Service and privacy consent is required.', 'fa' => 'تأیید رضایت و سیاست حفظ حریم خصوصی الزامی است.']]);
$limit = 2000;
$clean = [];
...
if (in_array($type, ['course', 'collaboration', 'personal_message', 'contact'], true) && $fullName === '') respond(422, ['message' => ['en' => 'Please enter your name.', 'fa' => 'لطفاً نام خود را وارد کنید.']]);
try {
    ...
    // Basic per-IP rate limit: block a 6th submission from the same visitor
    // within a 10-minute window. Uses the same salted ip_hash already stored
    // on every lead, so it needs no extra table or file storage.
    $ipHash = hash_hmac('sha256', (string)($_SERVER['REMOTE_ADDR'] ?? ''), (string)$config['ip_salt']);
    $rateStmt = $pdo->prepare('SELECT COUNT(*) FROM su_leads WHERE ip_hash = :ip_hash AND created_at > (UTC_TIMESTAMP() - INTERVAL 10 MINUTE)');
    $rateStmt->execute(['ip_hash' => $ipHash]);
    if ((int)$rateStmt->fetchColumn() >= 5) {
        respond(429, ['message' => ['en' => 'Too many requests. Please wait a few minutes and try again.', 'fa' => 'درخواست‌های زیادی ارسال شده. لطفاً چند دقیقه صبر کن و دوباره امتحان کن.']]);
    }
